...
class FEDD():
    ...
    def train_wrapper(self, data_train, for_training=True):
        ...
        qtd = 3
        vetor_caracteristicas_0 = self.FE(self.janela_caracteristicas.dados[:final-qtd])
        
        distancias_vetor = []
        for i in range(1, qtd):
            vetor_caracteristicas = self.FE(self.janela_caracteristicas.dados[i:final-qtd+i])
            distancia = self.computar_distancia(vetor_caracteristicas_0, vetor_caracteristicas)
            distancias_vetor.append(distancia)
            
        self.armazenar_conceito(vetor_caracteristicas_0, np.mean(distancias_vetor), np.std(distancias_vetor))
    ...
    def test_wrapper(self, stream):
        ...
        for i in range(len(stream)):
            if not self.drift_happened:
                ...
                vetor_caracteristicas_atual = self.FE(self.janela_caracteristicas.dados[0])    
                    
                #computar a distancia entre os vetores de caracteristicas
                distancia = self.computar_distancia(self.vetor_caracteristicas_inicial, vetor_caracteristicas_atual)   
                #distancias_vetor.append(distancia) 
