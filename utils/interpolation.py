...
def train_anomaly_removal(df_train):
    ...
    # creating a new df that replaces the anomalous samples with interpolation
    # value
    ...
    for i in range(0, len(df_train)):
        ...
        if ((df_train.is_anomaly[i] == 1) & (
                i != (len(df_train) - 1)) & (i != 0)):
            if (df_train.is_anomaly[i + 1] != 1):

                if ((interpolation_indexes(indexes, i)[
                        0] != -1) & (interpolation_indexes(indexes, i)[1] != -1)):
                    value_interpolation = (df_train.value[interpolation_indexes(indexes, i)[0]]
                                           + df_train.value[interpolation_indexes(indexes, i)[1]]) / 2

                    df = df.append(
                        {
                            'timestamp': df_train.timestamp[i],
                            'value': value_interpolation,
                            'is_anomaly': 0.0},
                        ignore_index=True)
    return df
